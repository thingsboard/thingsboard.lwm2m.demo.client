...
| `-u, --server-url`                         | Set the server URL. Defaults to `-u coap://localhost:5685` or `-u coaps://localhost:5686` or `-u coaps://localhost:5687` or `-u coaps://localhost:5688`.                                                                                                                              |
| `--u_b, --server-url-bs`                   | Set the  Additional URL for Bootstrap section, if you are configuring two servers at the same time. Defaults to `-u coap://localhost:5687` or `-u coaps://localhost:5688`.                                                                                                            |
| `-b, --bootstrap`                          | Use bootstrap mode instead of direct registration.                                                                                                                                                                                                                                    |
| `-lw_b, --lwm2m_bootstrap`                 | Use direct registration and additional features with the bootstrap section.                                                                                                                                                                                                           |
...
| `discover`                                         | List available Objects, Instances and Resources. For example:`discover` (is the equivalent of ThingsBoard's DiscoveryAll operation) or `discover 5` for ObjectId.                                                   |
...
#### 🔹 Run in NoSec mode (URL = local server: localhost; port = 5687) + connection to bootstrap server

```bash
java -jar thingsboard-lwm2m-demo-client-4.1.3.jar -u coap://localhost:5687 -b -n MyClientNoSec
```

#### 🔹 Run in NoSec mode (URL = local server: localhost; port = 5685) + connection to lwm2m server with Additional URL for Bootstrap section

```bash
java -jar thingsboard-lwm2m-demo-client-4.1.3.jar -u coap://localhost:5685  -u_b coap://localhost:5687 -lw_b -n MyClientNoSec
```
