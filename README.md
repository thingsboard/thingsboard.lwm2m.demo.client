...
| `-ic, --interactive-console`                | Use Java-CoAP instead of Californium. Syntax example:`-jc`.                                                                                                                                                                                                                            |
...
```xml
<configuration>
        <appender name="CLI" class="org/thingsboard/lwm2m/demo/client/logging/ConsoleAppenderCLI">
                <encoder>
...
**Note:**: 
- The dynamic commands from the Interactive Console mode only works when the **-icon**, **--interactive-console** option is _enabled_. 

```shell
 docker run --rm -it thingsboard/tb-lwm2m-demo-client:latest -u coap://demo.thingsboard.io -n MyClientNoSec -ic
```
